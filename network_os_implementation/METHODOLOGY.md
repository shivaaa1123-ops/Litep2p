...
- [x] Phase 3 — Durable Network Object Store (`03_phase_3_durable_network_object_store.md`) — **COMPLETE** (commit `019c40b` + re-validation fixes; `object/` ObjectID + signed immutable/mutable envelope + E2E key model, `objectstore/` SQLite/WAL store (transactions, hierarchical quotas, TTL, dedup-before-work, score eviction, crash-safe open, forward-migration guard), runtime owns store at `files_dir/networkos.sqlite`; envelope 131 checks × 10, store 63 checks × 10, SIGKILL harness 10/10, 13 suites × 5 = 0 failures, live two-node smoke 6/6, native build green, C ABI 56 functions identical, fuzz 60s no crash; re-validation audit fixed open()-self-deadlock + O(N²) quota/dedup scans → put 248→3,734 obj/s at 10k)
- [x] Phase 4 — Confirmed Remote Storage (`04_phase_4_confirmed_remote_storage.md`) — **COMPLETE** (two-phase durable handoff OBJECT_OFFER/ACCEPT/REJECT/DATA/STORED_ACK in the existing codec + handoff module `modules/networkos/handoff/`: signed replica leases (Ed25519, accepted_until ≤ object TTL, pressure/charging hints), never-ACK-before-commit (object+dedup+usage+lease in one transaction), idempotent replay, honest rejection (AUTH/POLICY/QUOTA/BUSY/RETRY_AFTER), schema v2 (leases + evicted_early + envelope_blob + REMOTE_ACCEPTED/DURABILITY_REACHED), ResourceManager stub, runtime wiring with retryPending on peer_ready; handoff_test 104 checks × 10, SIGKILL harness 10/10, live S→C handoff + restart proof 6/6, tcpdump two-phase flow 1/1, 14 suites × 5 = 0 failures, native build green, C ABI 56 identical, fuzz 12.7M iters no crash)
- [x] Phase 5 — Direct Delivery + Receipts (`05_phase_5_direct_delivery_and_receipts.md`) — **COMPLETE** (delivery module `modules/networkos/delivery/`: `RECEIVED_ACK` (0x39) + signed-receipt codec, DeliveryManager with direct/forward/origin-destination roles, cheapest-path dispatch, signed receipt reverse-path, idempotent repeat (invariant 3/18), replica release §64, failure classes §61); store schema v3 (receipts table + delivered/confirmed/failure_class columns + v2→v3 migration); runtime owns `delivery()` + typed-frame routing (handoff=storage vs delivery=destination==me) + forwardPending on peer_ready; delivery_test 60 checks × 5, 14 suites × 5 green, handoff 104/object_store 63/runtime 941 green, native build green, C ABI 56 identical; live/tcpdump harness in `tools/harness/p5_milestone_scenario.sh`)
- [x] Phase 6 — Anti-Entropy (`06_phase_6_anti_entropy.md`) — **COMPLETE** (INVENTORY (0x3A) / OBJECT_WANT (0x3B) strict+bounded codec, `IInventorySource` seam (exact-list v0, Bloom v1.5 hook), AntiEntropyManager pull-heavy reconciliation (inventory exchange → WANT missing only → prioritized/bounded transfer → Phase 7 repair stub), per-session object/byte/time caps, crash-reconnect convergence (invariants 3/4/6/13/18), telemetry counters, runtime owns `antiEntropy()` + routes INVENTORY/OBJECT_WANT + kicks sessions on peer_ready; anti_entropy_test 27 checks × 5, convergence with no-blind-resend verified via counters, 16-suite × 5 regression = 0 failures, Phase 5 §99 green, Android native build green, C ABI 56 identical, codec fuzz = `tools/anti_entropy_fuzz_smoke` 9.3M iters/60s no crash + `fuzz_wire_codec.cpp` libFuzzer coverage)
- [x] Phase 7 — Adaptive Replication (`07_phase_7_adaptive_replication.md`) — **COMPLETE** (`modules/networkos/replication/ReplicaPlanner`: target-driven repair to D0–D5 durability ladder, per-namespace `ReplicationPolicy` (no hardcoded constants), local peer scoring + trust tiers + failure-domain diversity placement, lease-expiry/eviction repair to target with NO over-replication, retry = backoff+jitter+event-triggered, connection budgets, telemetry; `ObjectStore` schema v4 (`objects.durability_level`, `repl_policies`, `peer_scores`, `replica_backoff` — v3→v4 migration; deadlock-free enumerated stores); runtime owns `replication()` + routes STORED_ACK→D1, RECEIVED_ACK→D5, LEASE_EXPIRING→repair, peer_ready/connectivity→retry; `replication_test` 34 checks green (churn baseline 50/50 → D2 = Phase 11 starting number), Phases 5 & 6 regression green, 18  suites green, `externalNativeBuildMultiThreadDebug` green (all ABIs); docs `docs/network-os/13-replication.md`)
...
- [x] Phase 11 — Simulator & Chaos Gates (`11_phase_11_simulator_and_chaos_lab.md`) — **COMPLETE** (commit pending; `modules/networkos/metrics/ReliabilityMetrics` (§44: P(del before TTL), P(receipt), median/P95, replica survival, bytes/obj, wakeups/day, CPU ms/obj, energy model — bounded+thread-safe); `desktop/tools/churn_simulator.cpp` deterministic chaos lab running REAL ObjectStore+ReplicaPlanner per peer (exact ≤200 peers default, scaled hot-200 to 10k) with churn/loss/partition/latency/clock-skew/storage/refusal/corruption/malicious/reboot injections + push-on-connect & bounded inventory-propagation last-mile (§45 fidelity); 12 scenarios; JSON reports; `ReplicaPlanner::Config::jitter_seed` added for reproducible runs; `desktop/tests/invariants_test` = §93 manifest gate (20 invariants, 56 checks) green ×10; `parser_fuzz_smoke` covers capability/envelope/handoff/receipt/inventory/want/manifest — 0 crashes ×5 seeds; `tools/harness/chaos/` 10 adb scenarios (device-gated); regression sweep ×5 = 0 failures; native build green; report `docs/network-os/20-reliability-report.md`)
- [x] Phase 12 — Public SDK & Compatibility (`12_phase_12_public_general_sdk.md`) — **COMPLETE** (commit `this`; additive public surface for the Network OS: C ABI `litep2p_nos_*` (register_namespace/send-with-clamped-policy/cancel/status/diagnostics/wire-version + `LITEP2P_FEATURE_NETWORK_OS`), Kotlin `NetworkOs` facade (thin, `deliveryEvents` SharedFlow, `SendResult`), JNI bridge + NativeEvents.onNosDeliveryEvent; runtime owns all logic (`Runtime::nosSend/nosCancel/nosStatusJson/nosRegisterNamespace/diagnosticsJson` — signed envelope, per-namespace clamps, replication-policy seam, FNV config fingerprint, privacy-safe snapshot); `desktop/tests/compat_test` (ABI snapshot gate vs frozen `c_abi_reference.txt` 64 symbols, version-negotiation matrix inv 13, v1-fixture→v4 migration data-intact + future-DB guard, policy clamps, diagnostics shape, opt-in live loopback) 32 checks ×5 = 0 failures; full regression ×5 = 0 failures; `externalNativeBuildMultiThreadDebug` green (platform-conditional adapter factory fix); Maven local both flavors + sources + dokka + portable zip refreshed; api-spec.md §16 (versioning contract, deprecation policy); device-gated DEFER: three-phone Appendix C scenario + §86 matrix + final bench (awaiting the two physical devices))
