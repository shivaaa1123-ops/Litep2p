...
public:
    ...
    // A separate Ed25519 keypair signs overlay FinalPayloads so the
    // destination can authenticate the origin. Stored next to the Noise key.
    void set_local_signing_keys(const std::vector<uint8_t>& public_key,
                                const std::vector<uint8_t>& secret_key);
    bool has_local_signing_keys() const;
    std::vector<uint8_t> get_local_signing_public_key() const;
    std::vector<uint8_t> get_local_signing_private_key() const;
    bool generate_and_store_local_signing_keys();

    void register_peer_signing_key(const std::string& peer_id,
...
private:
    ...
    // Origin-signing keys (Ed25519): 32B public, 64B secret.
    std::vector<uint8_t> m_local_signing_private_key;
    std::vector<uint8_t> m_local_signing_public_key;

    // Registered peer static public keys
    std::map<std::string, std::vector<uint8_t>> m_peer_keys;
    // Registered peer Ed25519 signing public keys (32 bytes each).
    std::map<std::string, std::vector<uint8_t>> m_peer_signing_keys;
