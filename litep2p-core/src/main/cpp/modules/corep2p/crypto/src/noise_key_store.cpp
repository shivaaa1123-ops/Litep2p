    ...
    try {
        ...
        if (in.good()) {
            ...
            if (j.is_object()) {
                ...
                // Origin-signing keys (Ed25519, Phase B).
                if (auto it = j.find("local_signing_pk_hex"); it != j.end() && it->is_string()) {
                    m_local_signing_public_key = hex_to_bytes(it->get<std::string>());
                }
                if (auto it = j.find("local_signing_sk_hex"); it != j.end() && it->is_string()) {
                    m_local_signing_private_key = hex_to_bytes(it->get<std::string>());
                }
                if (m_local_signing_public_key.size() == 32 &&
                    m_local_signing_private_key.size() == 64) {
                    loaded_anything = true;
                } else {
                    m_local_signing_public_key.clear();
                    m_local_signing_private_key.clear();
                }
                if (auto it = j.find("peer_signing_keys"); it != j.end() && it->is_object()) {
                    for (auto& kv : it->items()) {
                        if (!kv.value().is_string()) continue;
                        auto key = hex_to_bytes(kv.value().get<std::string>());
                        if (key.size() != 32) continue;
                        m_peer_signing_keys[kv.key()] = std::move(key);
                    }
                }
            }
...
bool NoiseKeyStore::save() {
    ...
    j["local_signing_pk_hex"] = bytes_to_hex(m_local_signing_public_key);
    j["local_signing_sk_hex"] = bytes_to_hex(m_local_signing_private_key);
    json peers = json::object();
    for (const auto& kv : m_peer_keys) {
        peers[kv.first] = bytes_to_hex(kv.second);
    }
    j["peer_keys"] = std::move(peers);
    json peer_signers = json::object();
    for (const auto& kv : m_peer_signing_keys) {
        peer_signers[kv.first] = bytes_to_hex(kv.second);
    }
    j["peer_signing_keys"] = std::move(peer_signers);
...
void NoiseKeyStore::set_local_signing_keys(const std::vector<uint8_t>& public_key,
                                           const std::vector<uint8_t>& secret_key) {
    if (public_key.size() != 32 || secret_key.size() != 64) {
        nativeLog("ERROR: KeyStore: Invalid signing key sizes (pk=" +
                  std::to_string(public_key.size()) + ", sk=" +
                  std::to_string(secret_key.size()) + ")");
        return;
    }
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        m_local_signing_public_key = public_key;
        m_local_signing_private_key = secret_key;
        m_dirty = true;
    }
    save();
}

bool NoiseKeyStore::has_local_signing_keys() const {
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_local_signing_public_key.size() == 32 &&
           m_local_signing_private_key.size() == 64;
}

std::vector<uint8_t> NoiseKeyStore::get_local_signing_public_key() const {
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_local_signing_public_key;
}

std::vector<uint8_t> NoiseKeyStore::get_local_signing_private_key() const {
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_local_signing_private_key;
}

bool NoiseKeyStore::generate_and_store_local_signing_keys() {
    if (sodium_init() < 0) {
        nativeLog("ERROR: KeyStore: libsodium init failed while generating signing keys");
        return false;
    }
    std::vector<uint8_t> pk(crypto_sign_PUBLICKEYBYTES, 0);
    std::vector<uint8_t> sk(crypto_sign_SECRETKEYBYTES, 0);
    if (crypto_sign_keypair(pk.data(), sk.data()) != 0) {
        nativeLog("ERROR: KeyStore: crypto_sign_keypair failed");
        return false;
    }
    set_local_signing_keys(pk, sk);
    return true;
}

void NoiseKeyStore::register_peer_signing_key(const std::string& peer_id,
                                              const std::vector<uint8_t>& public_key) {
    if (public_key.size() != crypto_sign_PUBLICKEYBYTES) {
        nativeLog("ERROR: KeyStore: Peer signing key must be 32 bytes");
        return;
    }
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        m_peer_signing_keys[peer_id] = public_key;
        m_dirty = true;
    }
    nativeLog("KeyStore: Peer signing key registered for " + peer_id);
    save();
}

std::vector<uint8_t> NoiseKeyStore::get_peer_signing_key(const std::string& peer_id) const {
    std::lock_guard<std::mutex> lock(m_mutex);
    auto it = m_peer_signing_keys.find(peer_id);
    return it == m_peer_signing_keys.end() ? std::vector<uint8_t>{} : it->second;
}
