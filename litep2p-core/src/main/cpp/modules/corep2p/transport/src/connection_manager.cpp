...
public:
    ...
    // Gracefully disconnect a single peer: shut down the socket for both read
    // and write. The peer's handleClient thread observes recv()==0, exits its
    // loop, closes the fd, removes the client entry and fires m_on_disconnect —
    // i.e. the exact same teardown path as a remote-initiated close. We do NOT
    // erase from m_clients here to avoid racing with that thread.
    bool disconnectPeer(const std::string& network_id) {
        int sock = -1;
        {
            std::lock_guard<std::mutex> lock(m_mutex);
            auto it = m_clients.find(network_id);
            if (it == m_clients.end()) {
                LOG_INFO("TCP_DISCONNECT: No connection for " + network_id);
                return false;
            }
            sock = it->second;
        }
        LOG_INFO("TCP_DISCONNECT: Shutting down connection to " + network_id);
        ::shutdown(sock, SHUT_RDWR);
        return true;
    }
