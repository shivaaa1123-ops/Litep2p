...
public:
    ...
    // Push-based delivery: every flush() snapshot is also handed to this
    // callback (single-line JSON). Registered by the public C ABI to feed the
    // on_telemetry event. Thread-safe; the callback may be invoked from the
    // engine's maintenance thread and must not block.
...
private:
    ...
    // Push callback (guarded by m_mu).
    PushCallback m_push_callback;
};
