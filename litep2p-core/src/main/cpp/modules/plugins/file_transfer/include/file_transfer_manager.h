...
/**
 * Incoming file-transfer offer (receiver side). Nothing is written to disk
 * until accept_incoming_offer() is called for this transfer_id.
 */
...
using TransferOfferCallback = std::function<void(const IncomingFileOffer& offer)>;
...
public:
    ...
    /**
     * Offer a file to a peer (offer/accept protocol).
     *
     * Creates a SEND transfer in PENDING state and sends an OFFER control
     * message to the peer. No file data is transmitted until the peer accepts
     * (see handle_incoming_message ACCEPT). On accept the transfer moves to
     * IN_PROGRESS and chunk transmission begins automatically.
     *
     * @return Transfer ID on success, empty string on failure.
     */
    std::string offer_file(const std::string& file_path,
                           const std::string& peer_id,
                           TransferPriority priority = TransferPriority::NORMAL);

    /**
     * Accept a pending incoming offer and start receiving.
     *
     * Creates the RECEIVE session (writing to save_path) and sends ACCEPT to
     * the offering peer. Nothing is written to disk before this call.
     *
     * @return true when the offer was known and accepted.
     */
    bool accept_incoming_offer(const std::string& transfer_id,
                               const std::string& save_path);

    /**
     * Decline a pending incoming offer; sends DECLINE to the offering peer.
     * @return true when the offer was known and declined.
     */
    bool decline_incoming_offer(const std::string& transfer_id);

    /**
     * List currently pending incoming offers (transfer ids).
     */
    std::vector<std::string> get_pending_offers() const;
    ...
    /**
     * Register incoming-offer callback (receiver side). Invoked when an OFFER
     * control message arrives; accept with accept_incoming_offer() or decline
     * with decline_incoming_offer().
     */
    void on_transfer_offered(TransferOfferCallback callback) {
        m_offer_callback = callback;
    }
...
private:
    ...
    TransferOfferCallback m_offer_callback;

    // Pending incoming offers (receiver side), keyed by transfer_id.
    std::unordered_map<std::string, IncomingFileOffer> m_pending_offers;
    mutable std::mutex m_offers_mutex;
    ...
    // Local (non-propagating) transfer control. The public pause/resume/cancel
    // methods apply the state change AND notify the remote peer; these internal
    // variants apply only the local state change and are used by the inbound
    // control-message handlers to avoid echoing a control frame back to the
    // peer that just sent it (which would ping-pong forever).
    bool pause_transfer_local(const std::string& transfer_id);
    bool resume_transfer_local(const std::string& transfer_id);
    bool cancel_transfer_local(const std::string& transfer_id);

    // Send a control frame (PAUSE/RESUME/CANCEL) for a transfer to its peer.
    void send_control_to_peer(const std::string& transfer_id, FtControlType type);
