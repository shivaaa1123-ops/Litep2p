...
// Parses "[id_len][id]" starting at offset; advances offset past the id.
static bool parse_id_field(const uint8_t* data, size_t len, size_t& offset, std::string& out_id) {
    if (offset + 1 > len) return false;
    uint8_t id_len = data[offset++];
    if (id_len == 0 || id_len > 128) return false;
    if (offset + id_len > len) return false;
    out_id.assign(reinterpret_cast<const char*>(data + offset), id_len);
    offset += id_len;
    return true;
}
...
void FileTransferManager::handle_incoming_message(const std::string& peer_id, std::string_view payload) {
    ...
    if (msg_type == 1) { // CHUNK
    ...
    } else if (msg_type == static_cast<uint8_t>(FtControlType::OFFER)) {
        // OFFER: [id][name_len][name][size u64][priority u8]
        std::string transfer_id;
        if (!parse_id_field(data, len, offset, transfer_id)) return;

        if (offset + 1 > len) return;
        uint8_t name_len = data[offset++];
        if (offset + name_len > len) return;
        std::string file_name(reinterpret_cast<const char*>(data + offset), name_len);
        offset += name_len;

        if (offset + 8 > len) return;
        uint64_t file_size = read_u64_le(data + offset);
        offset += 8;

        TransferPriority priority = TransferPriority::NORMAL;
        if (offset + 1 <= len) {
            const uint8_t p = data[offset];
            if (p <= static_cast<uint8_t>(TransferPriority::HIGH)) {
                priority = static_cast<TransferPriority>(p);
            }
        }
...

        {
            ...
            m_pending_offers[transfer_id] = offer;
        }

        LOG_INFO("FT: Incoming file offer: " + transfer_id + " (" + file_name + ", " +
                 std::to_string(file_size) + " bytes) from " + peer_id);

        if (m_offer_callback) {
            m_offer_callback(offer);
        }
    } else if (msg_type == static_cast<uint8_t>(FtControlType::ACCEPT)) {
        // ACCEPT: sender starts transmitting a PENDING offer.
        std::string transfer_id;
        if (!parse_id_field(data, len, offset, transfer_id)) return;
        ...
        bool started = false;
        {
            ...
            auto it = m_transfers.find(transfer_id);
            if (it != m_transfers.end() &&
                it->second->direction == TransferDirection::SEND &&
                it->second->state == TransferState::PENDING) {
        ...
        if (started) {
            LOG_INFO("FT: Offer accepted by peer, starting transfer: " + transfer_id);
            m_send_cv.notify_all();
        } else {
            LOG_WARN("FT: ACCEPT for unknown/non-pending transfer: " + transfer_id);
        }
    } else if (msg_type == static_cast<uint8_t>(FtControlType::DECLINE)) {
        // DECLINE: sender marks the offered transfer as cancelled.
        std::string transfer_id;
        if (!parse_id_field(data, len, offset, transfer_id)) return;
        ...
        TransferCompleteCallback complete_cb;
        {
            ...
            if (it != m_transfers.end() &&
                ...
                it->second->last_error = "declined by peer";
                complete_cb = m_complete_callback;
            }
        }
        LOG_INFO("FT: Offer declined by peer: " + transfer_id);
        if (complete_cb) {
            complete_cb(transfer_id, false, "declined by peer");
        }
    } else if (msg_type == static_cast<uint8_t>(FtControlType::CANCEL)) {
        // CANCEL: remote side cancelled; cancel locally too (no echo back).
        std::string transfer_id;
        if (!parse_id_field(data, len, offset, transfer_id)) return;
        LOG_INFO("FT: Remote cancel received for " + transfer_id);
        cancel_transfer_local(transfer_id);
    } else if (msg_type == static_cast<uint8_t>(FtControlType::PAUSE)) {
        // PAUSE: remote side paused; pause locally (no echo back).
        std::string transfer_id;
        if (!parse_id_field(data, len, offset, transfer_id)) return;
        LOG_INFO("FT: Remote pause received for " + transfer_id);
        pause_transfer_local(transfer_id);
    } else if (msg_type == static_cast<uint8_t>(FtControlType::RESUME)) {
        // RESUME: remote side resumed; resume locally (no echo back).
        std::string transfer_id;
        if (!parse_id_field(data, len, offset, transfer_id)) return;
        LOG_INFO("FT: Remote resume received for " + transfer_id);
        resume_transfer_local(transfer_id);
    }
}
