...
// ============================================================================
// OFFER / ACCEPT CONTROL PROTOCOL
// ============================================================================
// Control subtypes carried in FILE_TRANSFER payloads (chunk=1, ack=2):
//   3=OFFER  4=ACCEPT  5=DECLINE  6=CANCEL  7=PAUSE  8=RESUME
// All integers little-endian. Strings are length-prefixed (u8 length).
...
std::string encode_control_with_id(FtControlType type, const std::string& transfer_id) {
    std::string out;
    out.reserve(2 + transfer_id.size());
...
std::string encode_offer_payload(const std::string& transfer_id, const std::string& file_name,
                                 uint64_t file_size, TransferPriority priority) {
    std::string out;
    out.reserve(2 + transfer_id.size() + 1 + file_name.size() + 8 + 1);
    out.push_back(static_cast<char>(FtControlType::OFFER));
    out.push_back(static_cast<char>(static_cast<uint8_t>(transfer_id.size())));
    out.append(transfer_id);
    const uint8_t name_len = static_cast<uint8_t>(std::min<size_t>(file_name.size(), 255));
    out.push_back(static_cast<char>(name_len));
    out.append(file_name.data(), name_len);
    append_u64_le(out, file_size);
    out.push_back(static_cast<char>(static_cast<uint8_t>(priority)));
    return out;
}

} // namespace

std::string FileTransferManager::offer_file(const std::string& file_path,
                                            const std::string& peer_id,
                                            TransferPriority priority) {
    // Validate the source file and obtain its size.
    uint64_t file_size = 0;
    std::string file_name;
    {
        std::ifstream file(file_path, std::ios::binary | std::ios::ate);
        if (!file.is_open()) {
            LOG_WARN("FT: Cannot open file for offer: " + file_path);
            return "";
        }
    ...
    if (!outbound) {
        LOG_WARN("FT: Cannot offer file - no outbound message callback registered");
        return "";
    }

    const std::string transfer_id = generate_transfer_id();

    // Create the SEND session in PENDING state: the send worker only transmits
    // chunks for IN_PROGRESS sessions, so nothing flows until the peer accepts.
    {
    ...
    // Send the OFFER control message.
    outbound(peer_id, encode_offer_payload(transfer_id, file_name, file_size, priority));

    LOG_INFO("FT: File offered: " + transfer_id + " (" + file_name + ", " +
             std::to_string(file_size) + " bytes) to " + peer_id);
    return transfer_id;
}

bool FileTransferManager::accept_incoming_offer(const std::string& transfer_id,
                                                const std::string& save_path) {
    IncomingFileOffer offer;
    {
        std::lock_guard<std::mutex> lock(m_offers_mutex);
        auto it = m_pending_offers.find(transfer_id);
        if (it == m_pending_offers.end()) {
            LOG_WARN("FT: accept_incoming_offer - unknown offer: " + transfer_id);
            return false;
        }
        offer = it->second;
    ...
    if (save_path.empty()) {
        LOG_WARN("FT: accept_incoming_offer - empty save path");
        return false;
    }

    // Create the receive session (preallocates the .part file).
    if (!receive_file(transfer_id, save_path, offer.peer_id, "", 0, offer.file_size)) {
        LOG_WARN("FT: accept_incoming_offer - receive_file failed for " + transfer_id);
        return false;
    }

    // Tell the sender to start transmitting.
    TransferOutboundMessageCallback outbound;
    {
        std::lock_guard<std::mutex> lk(m_send_mutex);
        outbound = m_outbound_callback;
    ...
    if (outbound) {
        outbound(offer.peer_id, encode_control_with_id(FtControlType::ACCEPT, transfer_id));
    }

    LOG_INFO("FT: Offer accepted: " + transfer_id + " -> " + save_path);
    return true;
}

bool FileTransferManager::decline_incoming_offer(const std::string& transfer_id) {
    IncomingFileOffer offer;
    {
        std::lock_guard<std::mutex> lock(m_offers_mutex);
        auto it = m_pending_offers.find(transfer_id);
        if (it == m_pending_offers.end()) {
            LOG_WARN("FT: decline_incoming_offer - unknown offer: " + transfer_id);
            return false;
        }
        offer = it->second;
        m_pending_offers.erase(it);
    ...
    if (outbound) {
        outbound(offer.peer_id, encode_control_with_id(FtControlType::DECLINE, transfer_id));
    }

    LOG_INFO("FT: Offer declined: " + transfer_id);
    return true;
}

std::vector<std::string> FileTransferManager::get_pending_offers() const {
    std::lock_guard<std::mutex> lock(m_offers_mutex);
    std::vector<std::string> ids;
    ids.reserve(m_pending_offers.size());
    for (const auto& kv : m_pending_offers) {
...
bool FileTransferManager::pause_transfer_local(const std::string& transfer_id) {
    std::lock_guard<std::mutex> lock(m_transfers_mutex);
...
bool FileTransferManager::pause_transfer(const std::string& transfer_id) {
    // Idempotent: if the transfer is already paused (e.g. a remote PAUSE was
    // already applied via the local-only path), succeed without re-sending a
    // control frame. This prevents redundant frames / echo loops.
    {
        std::lock_guard<std::mutex> lock(m_transfers_mutex);
        auto it = m_transfers.find(transfer_id);
        if (it == m_transfers.end()) {
    ...
    if (!pause_transfer_local(transfer_id)) return false;
    send_control_to_peer(transfer_id, FtControlType::PAUSE);
    return true;
}

bool FileTransferManager::resume_transfer_local(const std::string& transfer_id) {
    std::lock_guard<std::mutex> lock(m_transfers_mutex);
...
bool FileTransferManager::resume_transfer(const std::string& transfer_id) {
    // Idempotent: if the transfer is already in progress (e.g. a remote RESUME
    // was already applied via the local-only path), succeed without re-sending
    // a control frame. This prevents redundant frames / echo loops.
    {
        std::lock_guard<std::mutex> lock(m_transfers_mutex);
        auto it = m_transfers.find(transfer_id);
        if (it == m_transfers.end()) {
    ...
    if (!resume_transfer_local(transfer_id)) return false;
    send_control_to_peer(transfer_id, FtControlType::RESUME);
    return true;
}

void FileTransferManager::send_control_to_peer(const std::string& transfer_id, FtControlType type) {
    std::string peer_id;
    {
        std::lock_guard<std::mutex> lock(m_transfers_mutex);
        auto it = m_transfers.find(transfer_id);
    ...
    payload.push_back(static_cast<char>(static_cast<uint8_t>(transfer_id.size())));
    payload.append(transfer_id);
...
bool FileTransferManager::cancel_transfer(const std::string& transfer_id) {
    if (!cancel_transfer_local(transfer_id)) return false;
    send_control_to_peer(transfer_id, FtControlType::CANCEL);
    return true;
}
