...
void UnifiedEventLoop::pushEvent(SessionEvent event) {
    ...
    }
    {
        ...
        if (m_event_queue.size() >= max_events) {
            const uint64_t dropped = m_dropped_events.fetch_add(1, std::memory_order_relaxed) + 1;
            if (dropped == 1 || (dropped % 100) == 0) {
                LOG_WARN("UnifiedEventLoop: event queue full; dropped=" + std::to_string(dropped));
            }
            return;
        }
