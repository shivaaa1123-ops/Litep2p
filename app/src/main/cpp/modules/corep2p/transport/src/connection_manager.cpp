...
public:
    ...
    bool connectToPeer(const std::string& ip, int port) {
        ...
        // Notify the session manager that we've connected
        if (m_on_data) {
    ...
    void sendMessageToPeer(const std::string& network_id, const std::string& message) {
        std::lock_guard<std::mutex> lock(m_mutex);
           if (m_clients.count(network_id)) {
              ...
              if (bytes_sent < encrypted_msg.size()) {
        ...
        while (m_running) {
            ...
            if (n > 0) {
                ...
                if (!decrypted_data.empty() && m_on_data) {
                    // Enhanced logging: show IP and port for received TCP connections
                    nativeLog("TCP: Received message from peer " + network_id + " (IP:Port) with payload: " + decrypted_data);
