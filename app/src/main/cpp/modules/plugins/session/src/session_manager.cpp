...
void SessionManager::Impl::setup_signaling_callbacks(SignalingClient& client) {
    client.setMessageCallback([this](const std::string& msg) {
        ...
        if (type == "REGISTER_ACK") {
            ...
            if (m_signaling_client && m_signaling_bootstrap_requested.load(std::memory_order_acquire)) {
                m_signaling_client->sendListPeers();
            }
            return;
        }
        ...
        if (type == "PEER_LIST") {
            ...
            if (!first_connectable_peer.empty() &&
                m_signaling_bootstrap_requested.exchange(false, std::memory_order_acq_rel)) {
...
void SessionManager::Impl::ensure_signaling_connected_async(bool force) {
    ...
    if (m_signaling_reconnect_in_progress.exchange(true, std::memory_order_acq_rel)) {
        return;
    }
    ...
    std::thread([this, url, local_peer_id]() {
