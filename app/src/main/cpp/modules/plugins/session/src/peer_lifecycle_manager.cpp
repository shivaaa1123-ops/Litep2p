            ...
            if (existing_peer != nullptr) {
                ...
                if (existing_peer->network_id != event.networkId) {
                    ...
                    if (is_ephemeral && new_is_private && prior_state == PeerState::CONNECTING) {
                        ...
                        // Peer is connected but discovery shows different port.
                        // Don't immediately tear down the session - it might be a duplicate ID or transient issue.
                        // Let the existing recovery mechanisms handle it:
                        // - CONTROL_CONNECT from new port will clear stale session (message_handler.cpp:307-322)
                        // - Decrypt failure will trigger session cleanup (message_handler.cpp:485-525)
                        LOG_INFO("SM: Peer " + event.peerId + " is CONNECTED to " + existing_peer->network_id + 
                                 " but discovered on " + event.networkId + " - possible restart/duplicate, waiting for CONTROL_CONNECT or decrypt failure");
                    } else {
                        ...
                        // updated endpoint. This fixes scenarios where signaling provides a WAN endpoint
                        // but LAN discovery later reveals the reachable private endpoint.
