...
public:
    DiscoveryImpl() : m_running(false), m_sock(-1), m_use_central_discovery(true) {}
            ...
            if (n > 0) {
                ...
                if (msg.rfind(DISCOVERY_MESSAGE_PREFIX, 0) == 0) {
                    ...
                    if (m_callback) {
                        ...
                        // Create network ID as IP:PORT format
                        std::string network_id = std::string(sender_ip) + ":" + std::to_string(ntohs(from_addr.sin_port));
