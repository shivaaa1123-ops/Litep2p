...
std::vector<uint8_t> NoiseNKSession::generate_ephemeral_keypair() {
    ...
    // Generate ephemeral keypair using libsodium
    if (crypto_kx_keygen(pk) != 0) {
        nativeLog("ERROR: NK: Failed to generate ephemeral public key");
        return {};
    }
    ...
    m_local_ephemeral_sk.assign(sk, sk + 32);
    m_local_ephemeral_pk.assign(pk, pk + 32);
    
    nativeLog("NK: Ephemeral keypair generated (pk=" + hex_encode(m_local_ephemeral_pk).substr(0, 8) + "...)");
...
void NoiseNKSession::perform_dh(
    ...
    if (crypto_scalarmult(dh_result, secret_key.data(), public_key.data()) != 0) {
        nativeLog("ERROR: NK: ECDH operation failed");
        result.clear();
        return;
    }
    result.assign(dh_result, dh_result + 32);
}

void NoiseNKSession::derive_keys() {
    // Noise NK key derivation using HKDF-SHA256
    // ck (chaining key), h (hash) initialized with protocol name
    unsigned char ck[32];
    unsigned char h[32];

    // Initialize with Noise protocol constants
    std::string protocol_name = "Noise_NK_25519_ChaChaPoly_SHA256";
    std::string prologue = "";

    // h = SHA256(protocol_name || prologue)
    crypto_hash_sha256(h, (unsigned char*)protocol_name.data(), protocol_name.length());
    if (!prologue.empty()) {
        unsigned char temp[32];
        crypto_hash_sha256(temp, (unsigned char*)prologue.data(), prologue.length());
        // Mix prologu
        for (int i = 0; i < 32; i++) h[i] ^= temp[i];
    }

    // ck = SHA256(protocol_name || prologue)
    std::memcpy(ck, h, 32);

    // Mix responder static key into handshake
    {
        unsigned char temp[32];
        crypto_hash_sha256_state state;
        crypto_hash_sha256_init(&state);
        crypto_hash_sha256_update(&state, ck, 32);
        crypto_hash_sha256_update(&state, m_responder_static_pk.data(), 32);
        crypto_hash_sha256_final(&state, temp);
        std::memcpy(ck, temp, 32);
    }

    // Collect DH results: ee, es (initiator) or ee, es (responder)
    ...
    // Derive send/receive keys from all DH results
    unsigned char key_material[64];
    unsigned char temp[32];

    // HKDF-SHA256 to derive keys
    for (size_t i = 0; i < dh_results.size(); i += 32) {
        crypto_hash_sha256_state state;
        crypto_hash_sha256_init(&state);
        crypto_hash_sha256_update(&state, ck, 32);
        crypto_hash_sha256_update(&state, dh_results.data() + i, 32);
        crypto_hash_sha256_final(&state, temp);
        std::memcpy(ck, temp, 32);
    }
    ...
    // Expand to get both send and receive keys
    {
        ...
        std::memcpy(prk, ck, 32);

        // HMAC-SHA256 expansion
        crypto_hash_sha256_state state;
        crypto_hash_sha256_init(&state);
        crypto_hash_sha256_update(&state, prk, 32);
        crypto_hash_sha256_update(&state, (unsigned char*)"\x01", 1);
        crypto_hash_sha256_final(&state, temp);

        std::memcpy(key_material, temp, 32);

        crypto_hash_sha256_init(&state);
        crypto_hash_sha256_update(&state, prk, 32);
        crypto_hash_sha256_update(&state, key_material, 32);
        crypto_hash_sha256_update(&state, (unsigned char*)"\x02", 1);
        crypto_hash_sha256_final(&state, temp);

        std::memcpy(key_material + 32, temp, 32);
    }
...
std::vector<uint8_t> NoiseNKSession::chacha20poly1305_encrypt(
    ...
    if (crypto_aead_chacha20poly1305_encrypt(
        ...
        plaintext.data(),
        plaintext.size(),
        nullptr,  // No additional authenticated data
        0,
        nullptr,  // No secret key
        nonce.data(),
        key.data()
    ) != 0) {
        nativeLog("ERROR: NK: ChaCha20-Poly1305 encryption failed");
        return {};
    }
...
std::vector<uint8_t> NoiseNKSession::chacha20poly1305_decrypt(
    ...
    if (crypto_aead_chacha20poly1305_decrypt(
        ...
        nativeLog("ERROR: NK: ChaCha20-Poly1305 decryption/authentication failed");
        return {};
    }
...
std::pair<std::vector<uint8_t>, std::vector<uint8_t>> NoiseNKManager::generate_static_keypair() {
    ...
    std::vector<uint8_t> public_key(pk, pk + 32);
    std::vector<uint8_t> secret_key(sk, sk + 32);
