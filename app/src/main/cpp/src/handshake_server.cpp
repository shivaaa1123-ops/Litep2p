...
HandshakeAcceptor::HandshakeAcceptor(int port): port_(port), running_(false), sock_(-1) {}
HandshakeAcceptor::~HandshakeAcceptor(){ stop(); }
    ...
    worker_ = std::thread([this,cb](){
        while (running_) {
            ...
            std::thread([fd, raddr, cb](){
                ...
                if (cb) cb(fd, raddr);
                // caller is responsible for closing fd if needed
            }).detach();
        }
    });
    return true;
