...
if ! python3 -m pip --version >/dev/null 2>&1; then
  if command -v apt-get >/dev/null 2>&1; then
    if [[ "${DRY_RUN}" == "1" ]]; then
      echo "[dry-run] apt-get install python3-pip"
    else
      ${SUDO} apt-get update >/dev/null
      ${SUDO} apt-get install -y python3-pip >/dev/null
    fi
  else
    echo "[suite] ERROR: python3-pip missing and apt-get not available" >&2
    exit 2
  fi
fi
if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[dry-run] python3 -m pip install websockets"
else
  if [[ "${EUID}" -ne 0 ]]; then
    python3 -m pip install --quiet --user websockets >/dev/null
  else
